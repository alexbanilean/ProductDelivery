...
public class AdminService implements IAdminService {
    ...
    public void removeShop(String shopName) {
        Shop shopToRemove = data.getShops().stream()
                        .filter(shop -> shop.getName().equals(shopName))
                        .findFirst()
                        .orElse(null);

        data.removeShop(shopToRemove);
    }

    @Override
    public void removeProduct(String shopName, String productName) {
        Shop shopFound = data.getShops().stream()
                             .filter(shop -> shop.getName().equals(shopName))
                             .findFirst()
                             .orElse(null);

        Product product = shopFound.getProducts().stream()
                                   .filter(prod -> prod.getName().equals(productName))
                                   .findFirst()
                                   .orElse(null);

        shopFound.removeProduct(product);
    }
